...
export function NextWeekRecommendations({ snapshot, history, isPremium }: NextWeekRecommendationsProps) {
  const { top3, recs } = useMemo(() => {
    const metrics = [
    ...
    const recs: string[] = []
    top3.forEach((k) => {
      RECOMMENDATIONS[k].forEach((r) => recs.push(r))
    })
    return { top3, recs: recs.slice(0, 3) }
  }, [snapshot, history])
          ...
          {!isPremium && (
            <div className="absolute inset-0 bg-white/60 backdrop-blur-[2px] z-10 flex items-center justify-center rounded-[24px]">
              <div className="text-center">
                ...
                <p className="text-[15px] font-medium text-[#4a4a4a]">Разблокировать в Premium</p>
              </div>
            </div>
          )}
